...
* `spin_speed` (`double`, default: 0.0) - 叠加到底盘角速度上的固定旋转速度；输入速度指令中的 `angular.z` 会被保留
...
默认关闭时，NAV2 使用 `base_link`，`cmd_vel_watchdog` 直接订阅 `/cmd_vel_keyboard`。启用后，NAV2 使用 `base_link_fake`，并启动 `fake_vel_transform` 将 `/cmd_vel_keyboard` 转换为 `/cmd_vel_transformed` 后再输出到底盘。
