...
void IntensityVoxelLayer::onInitialize()
{
  ...
  const auto declared_size_z =
    static_cast<int>(node->declare_parameter(name_ + ".z_voxels", 16));
  size_z_ = std::clamp(declared_size_z, 1, VOXEL_BITS);
...
void IntensityVoxelLayer::activate()
{
...
void IntensityVoxelLayer::deactivate()
{
  if (voxel_pub_) {
